...
void CRegisterBar::Update(const CContext *pCtx, const CString &stat, const CContext *pOld /*= NULL*/, bool bDraw /*= TRUE*/)
{
  ...
  if (theApp.m_global.m_bProc6502==2 && !pCtx->mem16) 
	SetDlgItemWordHex(IDC_REGS_A, pCtx->a);
  else
	SetDlgItemByteHex(IDC_REGS_A, pCtx->a & 0xff);
  UpdateRegA(pCtx);

  if (theApp.m_global.m_bProc6502==2 && !pCtx->xy16) 
...
//  SetDlgItemByteHex(IDC_REGS_A,pCtx->a);
	bool word = (theApp.m_global.m_bProc6502==2 && !pCtx->emm && !pCtx->mem16);
...
//  SetDlgItemByteHex(IDC_REGS_X,pCtx->x);
  bool word = (theApp.m_global.m_bProc6502==2 && !pCtx->emm && !pCtx->xy16);
...
void CRegisterBar::ChangeRegister(int ID, int reg_no)
{
  ...
  {
    case 0:
		if ((theApp.m_global.m_bProc6502==2) && !ctx.emm && !ctx.mem16)
		  ctx.a = UINT16(num);
		else
		  ctx.a = UINT8(num);
      UpdateRegA(&ctx);
      break;
    case 1:
 	  if ((theApp.m_global.m_bProc6502==2) && !ctx.emm &&!ctx.xy16)
		ctx.x = UINT16(num);
	  else
        ctx.x = UINT8(num);
      UpdateRegX(&ctx);
