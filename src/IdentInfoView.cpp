...
int CALLBACK CIdentInfoView::CompareLVIFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
    ...
    switch (pView->m_nSortBy)	// spos�b sortowania identyfikator�w
    {
    ...
    case 1:	// wg. nazwy
        if (str1.GetLength()>8 && str1[8] == CAsm::LOCAL_LABEL_CHAR)	// etykieta lokalna?
            if (str2.GetLength()>8 && str2[8] == CAsm::LOCAL_LABEL_CHAR)
                ...
                return 1;	// zawsze 1 (bez wzgl�du na 'neg')
        else if (str2.GetLength()>8 && str2[8] == CAsm::LOCAL_LABEL_CHAR)
            return -1;	// zawsze -1
        else
            ret = str1.Compare(str2);	// por�wnanie dw�ch etykiet globalnych
        return ret * neg;
    ...
    case 3:	// wg. zasi�gu
        if (str1.GetLength()>8 && str1[8] == CAsm::LOCAL_LABEL_CHAR)
            if (str2.GetLength()>8 && str2[8] == CAsm::LOCAL_LABEL_CHAR)	// etykiety lokalne?
                ret = -str1.Compare(str2);	// por�wnanie ca�ych etykiet
            else
                ret = -1;	// pierwsza lokalna, druga globalna (por�wnywane etykiety)
        else if (str2.GetLength()>8 && str2[8] == CAsm::LOCAL_LABEL_CHAR)	// etykiety lokalne?
            ret = 1;	// pierwsza globalna, druga lokalna (por�wnywane etykiety)
        else
...
//    case -5:
    case 0:	// bez sortowania
        return 0;
    default:
        ASSERT(FALSE);	// z�a wart. parametr 'lParamSort' dla fn por�wnuj�cej
        return 0;
    }
}
