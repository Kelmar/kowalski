...
// Asembler dla mikroprocesor�w M65XX i M65C02
...
CLeksem &CLeksem::operator = (const CLeksem &leks)
{
    ...
    if (type == L_STR || type == L_IDENT || type == L_IDENT_N)	// leksem docelowy zawiera �a�cuch znak�w?
    {
        delete str;
        str = NULL;
    }
    ...
    if (leks.type == L_STR || leks.type == L_IDENT || leks.type == L_IDENT_N)	// leksem �r�d�owy zawiera �a�cuch znak�w?
        str = new CLString(*leks.str);	// zdublowanie �a�cucha znak�w
    return *this;
}
...
#ifdef _DEBUG
            if (str->GetLength())
                str->SetAt(0, 'X');		// zmiana tekstu dla ew. wisz�cych odwo�a�
#endif
            delete str;
        }
        break;
...
bool CIdentTable::insert(const CString &str, CIdent &ident)
{
    ...
    {
        ident = val;	// zwr�cenie starego elementu
        return false;
    }
}
...
bool CIdentTable::replace(const CString &str, const CIdent &ident)
{
    ...
    else
    {
        ...
        {
            val = ident;	// zast�pienie nowym elementem starej warto�ci zmiennej
            return true;
        }
...
CLeksem CAsm6502::get_string(TCHAR lim)		// wyodr�bnienie �a�cucha znak�w
{
...
bool CAsm6502::proc_instr(const CString &str, OpCode &code)
{
    ...
    case 'B':
        ...
        case 'P':
            ...
            break;
        case 'R':
            if (m_procType != ProcessorType::M6502 && _totupper(str[2]) == 'A')
                return code = C_BRA, true;
            else if (_totupper(str[2]) == 'K')
                return code = C_BRK, true;
            else if (m_procType == ProcessorType::WDC65816 && _totupper(str[2]) == 'L')  //% 65816
    ...
    case 'C':
        ...
        case 'M':
            ...
            break;
        case 'O':
            if (m_procType == ProcessorType::WDC65816 && _totupper(str[2]) == 'P')  //% 65816
    ...
    case 'J':
        ...
        else if (m_procType == ProcessorType::WDC65816 && _totupper(str[1]) == 'M' && _totupper(str[2]) == 'L')  //% 65816
            return code = C_JML, true;
        else if (m_procType == ProcessorType::WDC65816 && _totupper(str[1]) == 'S' && _totupper(str[2]) == 'L')  //% 65816
    ...
    case 'M':
        ...
        {
            if (m_procType == ProcessorType::WDC65816 && _totupper(str[2]) == 'N')  //% 65816
                return code = C_MVN, true;
            else if (m_procType == ProcessorType::WDC65816 && _totupper(str[2]) == 'P')  //% 65816
    ...
    case 'P':
        if (_totupper(str[1]) == 'E')
        {
            if (m_procType == ProcessorType::WDC65816 && _totupper(str[2]) == 'A')  //% 65816
                return code = C_PEA, true;
            else if (m_procType == ProcessorType::WDC65816 && _totupper(str[2]) == 'I')  //% 65816
                return code = C_PEI, true;
            else if (m_procType == ProcessorType::WDC65816 && _totupper(str[2]) == 'R')  //% 65816
            ...
            case 'X':
                if (m_procType != ProcessorType::M6502) return code = C_PHX, true;
            case 'Y':
                if (m_procType != ProcessorType::M6502) return code = C_PHY, true;
            }
    ...
    case 'R':
        ...
        case 'O':
            ...
            break;
        case 'M':
            if (m_procType == ProcessorType::WDC65C02 && _totupper(str[2]) == 'B')
    ...
    case 'S':
        ...
        case 'T':
            ...
            else if (m_procType != ProcessorType::M6502 && _totupper(str[2]) == 'Z')
    ...
    case 'T':
        ...
        case 'A':
            ...
            break;
        case 'C':
            if ((m_procType == ProcessorType::WDC65816) && _totupper(str[2]) == 'D')  //% 65816
                return code = C_TCD, true;
            if ((m_procType == ProcessorType::WDC65816) && _totupper(str[2]) == 'S')  //% 65816
                return code = C_TCS, true;
            break;
        case 'D':
            if ((m_procType == ProcessorType::WDC65816) && _totupper(str[2]) == 'C')  //% 65816
                return code = C_TDC, true;
            break;
        case 'R':
            if (m_procType != ProcessorType::M6502 && _totupper(str[2]) == 'B')
        ...
        case 'X':
            ...
            if ((m_procType == ProcessorType::WDC65816) && _totupper(str[2]) == 'Y')  //% 65816
                return code = C_TXY, true;
            break;
        case 'Y':
            if (_totupper(str[2]) == 'A')
                return code = C_TYA, true;
            if ((m_procType == ProcessorType::WDC65816) && _totupper(str[2]) == 'X')  //% 65816
                return code = C_TYX, true;
            break;
        }
        break;
    case 'W':
        if ((m_procType == ProcessorType::WDC65816) && _totupper(str[1]) == 'A' && _totupper(str[2]) == 'I')  //% 65816
            return code = C_WAI, true;
        if ((m_procType == ProcessorType::WDC65816) && _totupper(str[1]) == 'D' && _totupper(str[2]) == 'M')  //% 65816
            return code = C_WDM, true;
        break;

    case 'X':
        if ((m_procType == ProcessorType::WDC65816) && _totupper(str[1]) == 'B' && _totupper(str[2]) == 'A')  //% 65816
            return code = C_XBA, true;
        if ((m_procType == ProcessorType::WDC65816) && _totupper(str[1]) == 'C' && _totupper(str[2]) == 'E')  //% 65816
    ...
    Expr &expr_bit, Expr &expr_zpg)
{
    ...
    case CLeksem::L_HASH:			// argument natychmiastowy '#'
        ...
        if (m_procType != ProcessorType::M6502 && leks.type == CLeksem::L_COMMA)	// only for 65C02?
        {
            leks = next_leks();		// kolejny niepusty leksem
            if (leks.type == CLeksem::L_HASH)
            {
                move = (UINT8)expr.value;
                leks = next_leks();		// kolejny niepusty leksem
                ret = expression(leks, expr);
                if (ret)				// niepoprawne wyra�enie?
                    return ret;
...
CAsm6502::Stat CAsm6502::asm_instr_syntax_and_generate(CLeksem &leks, InstrType it, const CString *pLabel)
{
    ...
    case I_ORG:		// origin
    {
        ...
        if (!(m_procType == ProcessorType::WDC65816) && expr.inf == Expr::EX_LONG)	// za du�a warto��
            return ERR_NUM_LONG;
        if (m_procType == ProcessorType::WDC65816)
            originWrapped = false;
        ...
        else
        {
            if (markArea && pass == 2 && origin != -1)
                markArea->SetEnd(UINT32(origin - 1));
            origin = expr.value & mem_mask;
            if (markArea && pass == 2)
                markArea->SetStart(origin);
        }
        if (pass == 2 && listing.IsOpen())
            listing.AddCodeBytes((UINT32)origin);
        break;
    }
    ...
    {
        UINT32 cnt_org = origin;	// miejsce na bajt d�ugo�ci danych (tylko .STR)
        ...
        if (pass = 2 && listing.IsOpen())
            listing.AddBytes(UINT32(cnt_org), mem_mask, out->Mem(), origin - cnt_org);
    ...
    case I_DCB:		// declare block
    {
        ...
        if (pass == 2 && out)
        {
            ...
            if (len && listing.IsOpen())
                listing.AddBytes(UINT32(org - len), mem_mask, out->Mem(), len);
        }
        break;
    }
    ...
    case I_SET:				// przypisanie warto�ci zmiennej
    {
        ...
        if (ret)
            return ret;
        if (pass == 2 && listing.IsOpen())
            listing.AddValue(UINT32(expr.value));
        break;
    }
    ...
    case I_ROM_AREA:	// protected memory area
    {
        ...
        else if ((UINT32)addr_from.value > mem_mask)		// 65816
            return ERR_NUM_LONG;
        ...
        else if ((UINT32)addr_to.value > mem_mask)	// za du�a warto��?
            return ERR_NUM_LONG;
            ...
            case CLeksem::L_PROC_INSTR:	// processor order (opcode)
            {
                ...
                if (pass == 2 && out && debug)
                {
                    ...
                    {	// debug info for the first line of the macro containing the instr. 6502
                        ...
                        generate_debug((UINT32)origin, pMacro->GetFirstLineNo(), pMacro->GetFirstLineFileUID());
                    }
                    else
                        generate_debug((UINT32)origin, text->GetLineNo(), text->GetFileUID());
            ...
            default:
                ...
                else			// second pass
                {
                    if (expr.inf != Expr::EX_UNDEF)
                        ...
                        return ERR_UNDEF_EXPR;
                    if (listing.IsOpen())
                        listing.AddValue((UINT32)expr.value); //65816 does this need updating?
                }
                if (ret)
                    return ret;
                state = COMMENT;
...
const TCHAR *CRepeatDef::GetCurrLine(CString &str)	// odczyt aktualnego wiersza do powt�rki
{
    ASSERT(m_nLineNo >= 0);
...
void CAsm6502::asm_fin()
{
    ...
    if (markArea && markArea->IsStartSet())
        markArea->SetEnd(UINT32(origin - 1) & mem_mask);    // fix for last area not being marked.
}
...
//-----------------------------------------------------------------------------
static const UINT8 NA = 0x42;   // WDM on 65816


CAsm6502::Stat CAsm6502::chk_instr_code(OpCode &code, CodeAdr &mode, Expr expr, int &length)
{
    UINT8 byte;
    const UINT8(&trans)[C_ILL][A_NO_OF_MODES] = TransformTable(m_procType);
    ...
    if (m_procType == ProcessorType::WDC65816 && code == C_WDM && mode == A_ACC) // allow WDM in 65816 mode
    {
        length = 1 + 1;
        return OK;
    }
    else if (byte == NA)				// niedozwolony tryb adresowania?
    {
        ...
        case A_ZPG_X:	// zero page indexed X
            if (m_procType == ProcessorType::WDC65816) mode = A_ABSL_X;
            if (trans[code][mode] == NA)
                mode = A_ABS_X;
            break;
...
void CAsm6502::generate_code(OpCode code, CodeAdr mode, Expr expr, Expr expr_bit, Expr expr_zpg)
{
    ...
    if (mode == A_IMP2 && code == C_BRK)
        ...
        (*out)[origin] = TransformTable(m_procType)[code][mode]; // rozkaz
    ...
    case A_ZPG2:	// zero page dla SMB i RMB
    {
        (*out)[origin] = TransformTable(m_procType)[code][mode]; // rozkaz SMB lub RMB
    ...
    {
        (*out)[origin] = TransformTable(m_procType)[code][mode]; // rozkaz BBS lub BBR
        ASSERT(expr_bit.inf == Expr::EX_BYTE && abs(expr_bit.value) < 8);
    ...
    case A_ABSL_X:	// absolute Long indexed X
        ...
        (*out)[origin + 1] = UINT8(expr.value & 0xFF);
        (*out)[origin + 2] = UINT8((expr.value >> 8) & 0xFF);
        (*out)[origin + 3] = UINT8((expr.value >> 16) & 0xFF);
        if (listing.IsOpen())
    ...
    case A_RELL:
    {
        ...
        ASSERT(dist<128 && dist>-129);
        (*out)[origin + 1] = UINT8(dist & 0xFF);
        (*out)[origin + 2] = UINT8((dist >> 8) & 0xFF);
        if (listing.IsOpen())
            listing.AddCodeBytes(origin, (*out)[origin], (*out)[origin + 1], (*out)[origin + 2]);
        break;
    }
...
void CAsm6502::generate_debug(UINT32 addr, int line_no, FileUID file_UID)
{
    ASSERT(debug != NULL);
...
CAsm::Stat CAsm6502::generate_debug(InstrType it, int line_no, FileUID file_UID)
{
    ...
    case I_ASCIS:	// ascii + $80 ostatni bajt
    {
        ...
        CDebugLine dl(line_no, file_UID, (UINT32)origin, DBG_DATA);
        debug->AddLine(dl);
        break;
    }
...
void CAsm6502::CListing::AddCodeBytes(UINT32 addr, int code1/*= -1*/, int code2/*= -1*/, int code3/*= -1*/, int code4/*= -1*/)
{
    ASSERT(m_nLine != -1);	// plik musi by� otwarty
    TCHAR buf[32];
...
void CAsm6502::CListing::AddValue(UINT32 val)
{
    ASSERT(m_nLine != -1);	// plik musi by� otwarty
    TCHAR buf[32];
    //% Bug Fix 1.2.13.2 - remove extra space from list report
...
void CAsm6502::CListing::AddBytes(UINT32 addr, UINT16 mask, const UINT8 mem[], int len)
{
    ASSERT(m_nLine != -1);	// plik musi by� otwarty
    ASSERT(len > 0);
    TCHAR buf[32];
