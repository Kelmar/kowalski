...
UINT16 CSym6502::get_argument_address(bool bWrite)
{
    ...
    case A_ABSI:						// only JMP(xxxx) supports this addr mode
        ...
        if (!isM6502 && (addr & 0xFF) == 0xFF)	// LSB == 0xFF?
...
//	m_Log.Record(ci);
    ...
    case C_ADC:
        ...
        if (ctx.decimal)
        {
            ...
            ctx.zero = ((theApp.m_global.GetProcType() == ProcessorType::M6502) ? !!zero : !!zeroc);
            ctx.carry = !!carry;
            ctx.negative = !!negative;
            ctx.overflow = !!overflow;
            if (!(theApp.m_global.GetProcType() == ProcessorType::M6502)) //% bug Fix 1.2.12.1 - fix cycle timing
    ...
    case C_SBC:
        ...
        if (ctx.decimal)
        {
            ...
            ctx.overflow = !!overflow;
            ctx.negative = ((theApp.m_global.GetProcType() == ProcessorType::M6502) ? !!negative : !!negativec);
            ctx.zero = ((theApp.m_global.GetProcType() == ProcessorType::M6502) ? !!zero : !!zeroc);
    ...
    case C_ILL:
        ...
        if (theApp.m_global.GetProcType() != ProcessorType::M6502) //65C02 mode
        {
            arg = get_argument_value();
            extracycle = false;
            break;
