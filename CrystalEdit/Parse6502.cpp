...
extern CString GetDirectiveDesc(const CString &strDirective)
{
    ...
    else if (strDirective.CompareNoCase(_T(".LSTRING")) == 0 ||
        strDirective.CompareNoCase(_T(".LSTR")) == 0)
        return _T("#title#.LSTR .LSTRING#text#\nDirectives defining byte values using passed string argument."
    ...
    else if (strDirective.CompareNoCase(_T(".XWORD")) == 0 ||
        strDirective.CompareNoCase(_T(".DX")) == 0)
        return _T("#title#.XWORD .DX#text#\nDirectives defining 24-bit word values using passed arguments."
            ...
            "beta:  .XWORD alpha\n"
            "       .XWORD alpha-1, beta\n");
    else if (strDirective.CompareNoCase(_T(".DWORD")) == 0 ||
        strDirective.CompareNoCase(_T(".DDW")) == 0)
        return _T("#title#.DWORD .DDW#text#\nDirectives defining 32-bit word values using passed arguments."
    ...
    else if (strDirective.CompareNoCase(_T(".PARAMTYPE")) == 0)
        return _T("#title#.PARAMTYPE#text#\nDirective that returns the type of parameter referenced: 1= number, 2=string"
            "#syntax#"
    ...
    else if (strDirective.CompareNoCase(_T(".STRLEN")) == 0)
        return _T("#title#.STRLEN#text#\nDirective that returns the length of the reference string."
            "#syntax#"
    ...
    else if (strDirective.CompareNoCase(_T(".DEF")) == 0)
        return _T("#title#.DEF#text#\nDirective that returns 1 if a label has been defined and 0 if not."
            " "
            "#syntax#"
            "[<label>[:]] .BYTE .DEF(test)  ; \n"
            "[<label>[:]] .IF .DEF(test)    ; \n"
    ...
    else if (strDirective.CompareNoCase(_T(".PASSDEF")) == 0)
        return _T("#title#.PASSDEF#text# Needs further testing to determine function.\n"
            " "
            "#syntax#"
...
{
    CString AddMode(UINT8 cmd, OpCode inst, CodeAdr mode, ProcessorType procType);
...
{
    const UINT8 NA = 0x42;  //WDM on 65816
    CString strModes;
    bool bExt1 = false;
    bool bExt2 = false;
    bool bExt3 = false;
    ...
    for (int i = 0; i < A_NO_OF_MODES; ++i)
    {
        ...
        if (code != NA)
            strModes += _T("<pre> ") + AddMode(code, inst, CodeAdr(i), ProcessorType::M6502);
        else
        {
            ...
            if (codeC != NA)
            {
                ...
                {
                    bExt3 = true;
                    strModes += _T("<pre>³") + AddMode(code, inst, CodeAdr(i), ProcessorType::WDC65816);
                }
                else
                {
                    strModes += _T("<pre>¹") + AddMode(codeC, inst, CodeAdr(i), ProcessorType::WDC65C02);
                    bExt1 = true;
                }
            }
            else
