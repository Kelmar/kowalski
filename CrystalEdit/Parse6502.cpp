...
{
	CString AddMode(UINT8 cmd, OpCode inst, CodeAdr mode, UINT8 b65c02);
...
CString Instructions::GetModes(OpCode inst)
{
	...
	for (int i= 0; i < A_NO_OF_MODES; ++i)
	{
		...
		if (code != NA)
			strModes += _T("<pre> ") + AddMode(code, inst, CodeAdr(i), 0);
		else
		{
			...
			if (codeC != NA)
			{
				...
				{
					bExt3 = true;
					strModes += _T("<pre>³") + AddMode(code, inst, CodeAdr(i), 2);
				}
				else
				{				
					strModes += _T("<pre>¹") + AddMode(codeC, inst, CodeAdr(i), 1);
					bExt1 = true;
				}
			}
			else
